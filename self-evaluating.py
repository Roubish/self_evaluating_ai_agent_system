...
logging.basicConfig(filename=str(LOG_DIR / 'system.log'), level=logging.INFO,
...
def search_tool(query: str) -> str:
    logging.info(f"Search tool called: {query}")
...
def code_tool(code: str) -> str:
    logging.info(f"Code tool executed: {code}")
    try:
...
def db_tool(query: str) -> str:
    logging.info(f"DB tool called: {query}")
...
def worker(state: AgentState) -> AgentState:
    ...
    feedback = f"Behavior: {state.get('behavior_feedback', '')}\nReasoning: {state.get('reasoning_feedback', '')}\nReviewer: {state.get('reviewer_feedback', '')}"
    prompt = f"""
    Query: {state['query']}
    Plan: {state['plan']}
    Feedback: {feedback}
...
def behavior_evaluator(state: AgentState) -> AgentState:
    ...
    """
    eval_resp = llm.invoke(prompt).content
    # Parse score, decision, explanation (use structured output or string parse)
    state['behavior_feedback'] = eval_resp
    write_to_file("behavior_eval.txt", eval_resp)
    return state

# 4. Reasoning Evaluator
...
def router(state: AgentState):
    if (is_approved(state.get('behavior_feedback', '')) and
        is_approved(state.get('reasoning_feedback', '')) and
        is_approved(state.get('reviewer_feedback', ''))):
        return END
...
def system_score(state: AgentState, exec_time: float) -> int:
    score = 10
    if exec_time > 10: score -= 2
    if state.get('revisions', 0) > 2: score -= 2
    if state.get('tool_usage', 0) > 6: score -= 2
    if state.get('worker_calls', 0) > 3: score -= 2
    return max(0, score)
...
if __name__ == "__main__":
    ...
    initial_state: AgentState = {"query": query, "worker_calls": 0, "tool_usage": 0, 
                                 "revisions": 0, "reviewer_calls": 0}
    ...
    Worker Calls: {result.get('worker_calls')}
    Revisions: {result.get('revisions')}
    Tool Usage: {result.get('tool_usage')}
